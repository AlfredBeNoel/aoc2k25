...
of their ingredients are *fresh* and which are <span
title="No, this puzzle does not take place on Gleba. Why do you ask?">
*spoiled* </span> . When you ask how it works, they give you a copy of
their database (your puzzle input).
...
ingredient IDs `    3   ` , `    4   ` , and `    5   ` are all *fresh*
. The ranges can also *overlap* ; an ingredient ID is fresh if it is in
*any* range.

The Elves are trying to determine which of the *available ingredient
IDs* are *fresh* . In this example, this is done as follows:

-   Ingredient ID `     1    ` is spoiled because it does not fall into
    any range.
-   Ingredient ID `     5    ` is *fresh* because it falls into range
    `     3-5    ` .
-   Ingredient ID `     8    ` is spoiled.
-   Ingredient ID `     11    ` is *fresh* because it falls into range
    `     10-14    ` .
-   Ingredient ID `     17    ` is *fresh* because it falls into range
    `     16-20    ` as well as range `     12-18    ` .
-   Ingredient ID `     32    ` is spoiled.

So, in this example, *`     3    `* of the available ingredient IDs are
fresh.
